...
MutRange<byte> comp_heap_realloc(CoreData* core, MutRange<byte> memory, u64 size, u64 align, bool allow_huge) noexcept
{
	ASSERT_OR_IGNORE(memory.begin() >= core->heap.memory);
	...
	const u64 old_size = memory.count();

	const u64 old_align = count_trailing_zeros(reinterpret_cast<u64>(memory.begin()));

	if (align <= old_align)
	{
		// If the alignment requirements did not increase, we can potentially reuse
		// the old allocation.

		if (size <= old_size)
		{
			// If the requested size is no larger than the old size, we can just
			// return what remains of the allocation.

			if (old_size >> COMP_HEAP_MIN_ALLOCATION_SIZE_LOG2 != size >> COMP_HEAP_MIN_ALLOCATION_SIZE_LOG2)
			{
				// If the decrease in size frees up some allocation slots, we
				// need to track them via the freelists.

				byte* const free_begin = memory.begin() + ((size + COMP_HEAP_ZERO_ADDRESS_MASK) & ~COMP_HEAP_ZERO_ADDRESS_MASK);

				const u64 free_size = (old_size - size + COMP_HEAP_ZERO_ADDRESS_MASK) & ~COMP_HEAP_ZERO_ADDRESS_MASK; 

				comp_heap_add_to_freelist(core, MutRange<byte>{ free_begin, free_size });
			}

			return memory.mut_subrange(0, size);
		}
		else if (size <= COMP_HEAP_MAX_ALLOCATION_SIZE)
		{
			// Since our allocation is growing, we can only satisfy it without
			// reallocating if it is either at the end of the heap, or has
			// slack in its last slot.
			// We have to be careful with alignment here, since the memory may
			// end somewhere inside the last heap slot and still count as "at
			// the end".

			const byte* const heap_end = core->heap.memory + core->heap.used;

			const byte* const memory_end = memory.end();

			ASSERT_OR_IGNORE(heap_end >= memory_end);

			if (old_size + (reinterpret_cast<u64>(memory_end) & COMP_HEAP_ZERO_ADDRESS_MASK) >= size)
			{
				// We can fit the additional size into the old allocation's
				// slack, making all of this effectively a no-op.

				return MutRange<byte>{ memory.begin(), size };
			}
			else if (heap_end - memory_end < (1 << COMP_HEAP_MIN_ALLOCATION_SIZE_LOG2))
			{
				// We can grow the allocation, since it is at the end of the
				// heap.

				if (!comp_heap_ensure_commit(core, core->heap.used + size - old_size))
					return MutRange<byte>{};

				core->heap.used += size - old_size;
	...
	// Since either the required alignment grew, or we could not grow the
	// existing allocation in place, we need to allocate new memory and move
	// the old one's contents into it. 

	MutRange<byte> new_memory = comp_heap_alloc(core, size, align, allow_huge);

	const u64 initialized_size = old_size < size ? old_size : size;

	range::mem_copy(new_memory.mut_subrange(initialized_size), memory.subrange(initialized_size));
