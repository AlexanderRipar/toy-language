...
private:
	...
	u64 m_commit_increment;

	u64 m_reserved;

	void ensure_capacity(u64 extra_used) noexcept
	{
		...
		const u64 new_commit = next_multiple(required_commit, m_commit_increment);
...
public:
		...
		m_reserved{ reserve }
	{
		...
		ASSERT_OR_IGNORE((reserve & (allocation_granularity() - 1)) == 0);

		ASSERT_OR_IGNORE((commit_increment & (allocation_granularity() - 1)) == 0);
	...
	void append(const void* data, u64 count) noexcept
	{
		ensure_capacity(count);

		memcpy(m_memory + m_used, data, count);
	...
	static u32 allocation_granularity() noexcept
	{
		return minos::page_bytes();
	}

	void free_region(void* begin, u64 count) noexcept
	{
	...
	{
		free_region(begin, static_cast<byte*>(end) - static_cast<byte*>(begin));
	}
	...
	u64 used() const noexcept
	{
		return m_used;
	}

	u64 committed() const noexcept
	{
		return m_committed;
	}
