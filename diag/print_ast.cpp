...
static void print_node_header(FILE* out, IdentifierPool* identifiers, const a2::AstNode* node, s32 depth) noexcept
{
	if (node->tag == a2::AstTag::ValIdentifer)
	{
		...
		fprintf(out, "%*s%s [%.*s] {%s\n", (depth + 1) * 2, "", a2::ast_tag_name(node->tag), static_cast<s32>(name.count()), name.begin(), a2::has_children(node) ? "" : "}");
	}
	else if (node->tag == a2::AstTag::ValInteger)
	{
		fprintf(out, "%*s%s [%lld] {%s\n", (depth + 1) * 2, "", a2::ast_tag_name(node->tag), a2::attachment_of<a2::ValIntegerData>(node)->value, a2::has_children(node) ? "" : "}");
	}
	else
	{
		fprintf(out, "%*s%s {%s\n", (depth + 1) * 2, "", a2::ast_tag_name(node->tag), a2::has_children(node) ? "" : "}");
	}
}
