...
public:
	...
	bool init(MemorySubregion memory, Index initial_commit_bytes) noexcept
	{
		ASSERT_OR_IGNORE(memory.count() != 0 && memory.count() <= std::numeric_limits<Index>::max());

		ASSERT_OR_IGNORE(initial_commit_bytes != 0 && initial_commit_bytes <= memory.count());

		Index actual_initial_commit_bytes = impl::page_size();

		while (actual_initial_commit_bytes < initial_commit_bytes)
			actual_initial_commit_bytes *= 2;

		if (!memory.commit(0, actual_initial_commit_bytes))
			return false;
		...
		m_committed_bytes = actual_initial_commit_bytes;
	...
	bool grow() noexcept
	{
		if (m_committed_bytes == m_memory.count())
			return false;

		if (m_memory.commit(m_committed_bytes, m_committed_bytes))
			return false;
	...
	u64 committed_bytes() const noexcept
	{
		return m_committed_bytes;
	}
